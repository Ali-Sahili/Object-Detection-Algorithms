...
from models.detr import DETR, MLP
...
class UPDETR(DETR):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, aux_loss=False,
        ...
        Parameters:
            ...
            aux_loss: True if auxiliary decoding losses (loss at each decoder layer) are to be 
            used.
        ...
        if self.feature_recon:
            # align the transformer feature to the CNN feature, which is used for the feature 
            # reconstruction
    ...
    def forward(self, samples: NestedTensor, patches: torch.Tensor):
        ...
        if self.training:
            ...
            mask_query_patch = (torch.rand(self.num_queries, bs, 1, 
                                           device=patches.device) > self.mask_ratio).float()
            ...
            hs = self.transformer( self.input_proj(src), mask, patch_feature, pos[-1], 
                                   self.attention_mask.to(patch_feature.device))[0]
        else:
            ...
            hs = self.transformer( self.input_proj(src), mask, patch_feature, pos[-1], 
                     self.attention_mask.to(patch_feature.device)[:num_queries,:num_queries])[0]
        ...
        if self.feature_recon:
            ...
            if self.aux_loss:
                out['aux_outputs'] = self._set_aux_loss(outputs_class, outputs_coord, 
                                                        outputs_feature, patch_feature_gt)
        else:
    ...
    def _set_aux_loss(self, outputs_class, outputs_coord, outputs_feature, backbone_output):
        ...
        # The "ground truth" of the feature reconstruction is constructed during the model 
        # forward
...
def build_updetr(num_classes=2):
    ...
    transformer = Transformer(d_model=hidden_dim, normalize_before=True, 
                                                  return_intermediate_dec=True)
    model = UPDETR(backbone_with_pos_enc,
                   transformer,
